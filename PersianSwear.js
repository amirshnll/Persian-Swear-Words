...
var PersianSwear = {
    ...
    add_word: function(text) { // return nothing
        PersianSwear.initial();
        const found = words.find(e => e == text);
        if (found == undefined)
            words.push(text)
    },
    remove_word: function(text) { // return nothing
        PersianSwear.initial();
        words = words.filter(e => e != text);
    },
    is_bad: function(text) { // return boolean
        PersianSwear.initial();
        const found = words.find(e => e == text);
        if (found == undefined)
            return false;
    ...
    has_swear: function(text) { // return boolean
        PersianSwear.initial();
        for (let index = 0; index < words.length; index++)
            if (text.includes(words[index]))
                return true;
        return false;
    },
    filter_words: function(text,symbol="*") { // return string
        var text_result = "";
        var text_splited = text.split(" ");
        for (var i = 0; i < text_splited.length; i++) {
            if(PersianSwear.is_bad(text_splited[i]))
                text_result += symbol;
            else
